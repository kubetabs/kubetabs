...
import (
	...
	"kubetabs/handler"
	"kubetabs/handler/sd"
	_ "kubetabs/pkg/jwtauth"
	"kubetabs/router/middleware"
	"log"
)
...
func InitRouter() *gin.Engine {
    ...
    k8sv1 := r.Group("/api/v1")
    {
        k8sv1.GET("/k8s/clusterRoleList", k8s.GetClusterRoleList)
        k8sv1.GET("/k8s/nodeList", k8s.GetNodeList)
        k8sv1.GET("/k8s/node/:hostname", k8s.GetNode)
        k8sv1.GET("/k8s/nodeMetricsList", k8s.GetNodeMetricsList)
        k8sv1.GET("/k8s/nodeMetrics/:hostname", k8s.GetNodeMetrics)
        k8sv1.GET("/k8s/podList", k8s.GetPodList)
        k8sv1.GET("/k8s/podMetrics", k8s.GetPodMetrics)
        k8sv1.GET("/k8s/nsList", k8s.GetNsList)
        k8sv1.GET("/k8s/pvList", k8s.GetPvList)
        k8sv1.GET("/k8s/scList", k8s.GetScList)
        k8sv1.GET("/k8s/deploymentList", k8s.GetDeploymentList)
        k8sv1.GET("/k8s/replicaSetList", k8s.GetReplicaSetList)
        k8sv1.GET("/k8s/daemonSetList", k8s.GetDaemonSetList)
        k8sv1.GET("/k8s/statefulSetList", k8s.GetStatefulSetList)
    }
